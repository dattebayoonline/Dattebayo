...
// ----- STATS CALCULATION -----
	...
	if((skill=pc_checkskill(sd,AC_OWL1))>0)
		base_status->str += 10;
	if((skill=pc_checkskill(sd,AC_OWL2))>0)
		base_status->agi += 10;
	if((skill=pc_checkskill(sd,AC_OWL3))>0)
		base_status->vit += 10;
	if((skill=pc_checkskill(sd,AC_OWL4))>0)
		base_status->int_ += 10;
	if((skill=pc_checkskill(sd,AC_OWL5))>0)
		base_status->luk += 10;
	if((skill=pc_checkskill(sd,AC_OWL6))>0)
		base_status->luk += 10;
	if((skill=pc_checkskill(sd,AC_OWL7))>0)
		base_status->str += 20;
    	if((skill=pc_checkskill(sd,AC_OWL8))>0)
       		base_status->agi += 20;
    	if((skill=pc_checkskill(sd,AC_OWL9))>0)
        	base_status->vit += 20;;
    	if((skill=pc_checkskill(sd,AC_OWL10))>0)
		base_status->int_ += 20;
    	if((skill=pc_checkskill(sd,AC_OWL11))>0)
    	    	base_status->luk += 20;
    	if((skill=pc_checkskill(sd,AC_OWL12))>0)
      	  	base_status->luk += 20;
    	if((skill=pc_checkskill(sd,AC_OWL13))>0)
        	base_status->str += 30;
    	if((skill=pc_checkskill(sd,AC_OWL14))>0)
        	base_status->agi += 30;
    	if((skill=pc_checkskill(sd,AC_OWL15))>0)
        	base_status->vit += 30;;
    	if((skill=pc_checkskill(sd,AC_OWL16))>0)
		base_status->int_ += 30;
    	if((skill=pc_checkskill(sd,AC_OWL17))>0)
        	base_status->luk += 30;
   	if((skill=pc_checkskill(sd,AC_OWL18))>0)
        	base_status->luk += 30;
...
#else
			sc_def = status->luk*100;
			sc_def2 = (status->luk + status->luk) * 5 + status_get_lv(bl)*10 - status_get_lv(src)*10;
#endif
			tick_def2 = status->luk*10;
			break;
		case SC_STONE:
