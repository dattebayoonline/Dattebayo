...
BUILDIN_FUNC(guildjoin) {
	...
	if ( !( sd = map_charid2sd( script_getnum(st,3) ) ) ) {
		script_pushint(st,-1);
		ShowError("buildin_guildjoin: Invalid char_id argument!\n");
		return SCRIPT_CMD_FAILURE;
	}
	if ( sd->status.guild_id ) {
		script_pushint(st,-2);
		ShowError("buildin_guildjoin: player is already on a guild!\n");
		return SCRIPT_CMD_FAILURE;
	}
	if ( !( g = guild_search( guild_id ) ) ) {
		script_pushint(st,-3);
		ShowError("buildin_guildjoin: Error in guild_id argument! search guild not found.\n");
		return SCRIPT_CMD_FAILURE;
	}
	if ( g->max_member >= MAX_GUILD ) {
		script_pushint(st,-4);
		ShowError("buildin_guildjoin: guild is already full!.\n");
		return SCRIPT_CMD_FAILURE;
	}
